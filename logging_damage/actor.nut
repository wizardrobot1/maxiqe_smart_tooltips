this.actor <- this.inherit("scripts/entity/tactical/entity", {
	...
	{
        ::logWarning("Hit damage debugging: in actor.onDamageReceived. Logging _hitInfo:");
        ::MSU.Log.printData( _hitInfo );
        ...
        ::logWarning("Hit damage debugging: in actor.onDamageReceived. Logging p (defender info):");
        ::MSU.Log.printData( p );
        ...
        ::logWarning("Hit damage debugging: in actor.onDamageReceived. Logging _hitInfo (after initial subtraction):");
        ::MSU.Log.printData( _hitInfo );
		...
		if (_hitInfo.DamageDirect < 1.0)
		{
            ...
            ::logWarning("Inside of `_hitInfo.DamageDirect < 1.0` conditional; armorDamage = " + armorDamage);
		}
		...
		local damage = 0;
        local reduction = armor * this.Const.Combat.ArmorDirectDamageMitigationMult;
        local damage_before_reduction = _hitInfo.DamageRegular * _hitInfo.DamageDirect * p.DamageReceivedDirectMult;
		damage = damage + this.Math.maxf(0.0, damage_before_reduction - reduction);

        ::logWarning("After first damage stack; see info below");
        ::MSU.Log.printData({
            damage_before_reduction=damage_before_reduction,
            armor=armor,
            multiplier=this.Const.Combat.ArmorDirectDamageMitigationMult,
            reduction=reduction,
            damage=damage
        });
		...
		{
            local damage_batch_2 = _hitInfo.DamageRegular * this.Math.maxf(0.0, 1.0 - _hitInfo.DamageDirect * p.DamageReceivedDirectMult);
			damage = damage + this.Math.max(0, damage_batch_2 - armorDamage);

            ::logWarning("Inside of second damage conditional; see info below");
            ::MSU.Log.printData( {damage_batch_2=damage_batch_2, armorDamage=armorDamage, diif=damage_batch_2 - armorDamage, damage=damage} );
		}
        ...
        ::logWarning("Final damage; damage = " + damage);
